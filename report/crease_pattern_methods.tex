...
\footnotesize Render the exact tree pose and blend late into a separated flat
stack:
\[
P_f^{\mathrm{legacy}}(t)
=
\bigl(1-s(t)\bigr)P_f^{\mathrm{tree}}(t)+s(t)L_f.
\]
\end{block}

\begin{block}{Balanced stack}
\footnotesize Close seams by averaging shared vertices and rigidly refitting
each face, then blend to a thinner final stack:
\[
P_f^{\mathrm{bal}}(t)
=
\bigl(1-s(t)\bigr)\mathcal R(P^{\mathrm{tree}}(t))_f+s(t)B_f.
\]
\end{block}

\column{0.46\textwidth}
\begin{alertblock}{Seamless rigid}
\footnotesize At sample times \(t_j\), solve per-face rigid transforms by
\[
\min \lambda_s E_{\mathrm{seam}}
+\lambda_g E_{\mathrm{guide}}
+\lambda_h E_{\mathrm{hinge}},
\]
so seams close, faces stay near the exact guide pose, and neighboring faces
track the guide hinge rotation.

\smallskip
Accepted samples are interpolated by SLERP. If one sample fails, hold the
nearest solved exact sample instead of jumping to the older gappy fallback.
At \(t=1\), render the exact folded figure.
\end{alertblock}
\end{columns}
